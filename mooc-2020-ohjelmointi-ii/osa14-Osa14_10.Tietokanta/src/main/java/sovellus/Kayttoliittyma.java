    ...
    public void kaynnista() throws SQLException {
        while (true) {
            ...
            if (komento.equals("2")){
                ...
                Todo todo = new Todo(eka, toka, false);
                tietokanta.lisaa(new Todo(eka, toka, false));

            }
            if (komento.equals("3")){
                System.out.println("Mikä asetetaan tehdyksi (syötä id)?");
                String id = this.lukija.nextLine();
                tietokanta.asetaTehdyksi(Integer.valueOf(id));
                
            }
            if (komento.equals("4")){
                System.out.println("Mikä poistetaan (syötä id)?");
                String id = this.lukija.nextLine();
                tietokanta.poista(Integer.valueOf(id));
            }
        }
